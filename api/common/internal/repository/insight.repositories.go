...
type InsightRepository interface {
	SaveInsight(insight model.MonitorInsight) error
	GetInsightByMonitor(monitorID string) (*model.MonitorInsight, error)
}
...
func (db *insightRepository) SaveInsight(insight model.MonitorInsight) error {
...
func (db *insightRepository) GetInsightByMonitor(monitorID string) (*model.MonitorInsight, error) {
	collection := insightCollection(db.connection)
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	filter := bson.M{"monitorId": monitorID}
	var insight model.MonitorInsight
