...
type UserRepository interface {
	...
	UpdateUser(id string, user model.User) (*mongo.UpdateResult, error)
...
func (db *userRepository) GetUser(email string) (*model.User, error) {
...
func (db *userRepository) GetUsers() ([]*model.User, error) {
	collection := collectionHelper(db.connection)
	...
	defer func(cursor *mongo.Cursor, ctx context.Context) {
		...
		if err != nil {
			slog.Error("cursor decode failed", "error", err)
		}
	}(cursor, ctx)
...
func (db *userRepository) CreateUser(user model.User) (*mongo.InsertOneResult, error) {
	collection := collectionHelper(db.connection)
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (db *userRepository) UpdateUser(id string, user model.User) (*mongo.UpdateResult, error) {
	collection := collectionHelper(db.connection)
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
