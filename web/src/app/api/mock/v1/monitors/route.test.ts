...
describe("/v1/monitors CRUD", () => {
  ...
  it("creates a monitor as pending", async () => {
    const res = await POST(post({ name: "New check", target: "https://x.dev" }));
  ...
  it("enforces name_taken", async () => {
    const res = await POST(post({ name: "Homepage", target: "https://dup.dev" }));
  ...
  it("enforces timeout < interval", async () => {
    const res = await POST(
      post({ name: "Bad timing", target: "https://x.dev", interval_seconds: 30, timeout_seconds: 30 }),
    );
  ...
  it("mutes via PATCH and deletes via DELETE", async () => {
    const patched = await PATCH(
      new Request("http://mock", { method: "PATCH", body: JSON.stringify({ muted: true }) }),
      params("mon_homepage"),
    );
    expect((await patched.json()).muted).toBe(true);

    const deleted = await DELETE(new Request("http://mock"), params("mon_homepage"));
    expect(deleted.status).toBe(204);

    const missing = await DELETE(new Request("http://mock"), params("mon_homepage"));
