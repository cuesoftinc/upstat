...
export async function POST(req: Request) {
  const body = await readJson<Pick<SavedView, "name" | "scope" | "surface" | "query">>(req);
  if (!body?.name || !body.surface || !body.query) {
    return jsonError(422, "bad_shape", "name, surface and query are required");
  }
  const db = getDb();
  if (db.views.some((v) => v.surface === body.surface && v.name === body.name)) {
    return jsonError(409, "name_taken", "a view with this name exists on this surface");
  }
