...
export function substituteVars(query: string, vars: Record<string, string>): string {
  return query.replace(/\$([a-zA-Z_][a-zA-Z0-9_]*)/g, (m, name: string) => vars[name] ?? m);
}
...
export function useWidgetData(widget: Widget, vars: Record<string, string>): WidgetData {
  ...
  const query = useRequest<QueryResult>(
    () =>
      isQuery && resolvedQuery
        ? queryRepo.run({ q: resolvedQuery, from: time.fromIso, to: time.toIso })
  ...
  const logs = useRequest(
    () =>
      isLogs
        ? logsRepo.query({ q: resolvedQuery, from: time.fromIso, to: time.toIso, limit: 8 })
        : Promise.resolve({ events: [] as LogEvent[], histogram: [], facets: {} }),
    [isLogs, resolvedQuery, time.fromIso, time.toIso],
  );

  const slos = useRequest(() => (isSlo ? slosRepo.list() : Promise.resolve([] as Slo[])), [isSlo]);
...
export function seriesToHeatmap(series: { points: { ts: string; value: number | null }[] } | undefined): {
  columns: string[];
  rows: string[];
  values: number[][];
} {
  ...
  const values = rows.map((_, r) =>
    sampled.map((p, c) => {
      const v = vals[c];
      const bucketOfV = Math.min(bucketCount - 1, Math.floor((1 - v / max) * bucketCount));
...
export function defaultWidget(type: Widget["type"]): Omit<Widget, "id" | "dashboard_id"> {
  const base = {
    query: null,
  ...
  switch (type) {
    case "timeseries":
      return { ...base, type, title: "p95 latency by service", query_string: "metric:http.request.duration_ms | p95() by service", viz_options: { display: "line" } };
    case "query_value":
      return { ...base, type, title: "Requests /s", query_string: "metric:http.requests_total | rate()", viz_options: { sparkline: true }, layout: { ...base.layout, w: 3, h: 2 } };
    case "toplist":
      return { ...base, type, title: "Top services by errors", query_string: "metric:http.errors_total | sum() by service", viz_options: { limit: 5 } };
    case "table":
      return { ...base, type, title: "Services", query_string: "metric:http.requests_total | rate() by service" };
    case "heatmap":
      return { ...base, type, title: "Latency distribution", query_string: "metric:http.request.duration_ms | p95() by service" };
    case "logstream":
      return { ...base, type, title: "Error logs", query_string: "level:ERROR" };
    case "trace_latency":
      return { ...base, type, title: "Trace latency — api-common", query_string: "metric:http.request.duration_ms service:api-common | p50(), p95(), p99()" };
    case "slo":
      return { ...base, type, title: "SLOs", layout: { ...base.layout, w: 4, h: 2 } };
    case "status":
      return { ...base, type, title: "Uptime checks", layout: { ...base.layout, w: 4, h: 2 } };
    case "servicemap":
      return { ...base, type, title: "Service map" };
    case "markdown":
      return { ...base, type, title: "Notes", viz_options: { text: "Double-click Edit to write runbook notes here." }, layout: { ...base.layout, w: 4, h: 2 } };
  }
}
